<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8" %>
  <!DOCTYPE html>
  <html lang="en">

  <head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="assets/css/inicio_dp.css">    
    <title>MENU DP</title>
  </head>

  <body>
    <div class="contenedor"> 
      <!--contenidos del header -->
      <div class="header"> 
        <!-- imagen con la accion de devolver a el index-->
        <div id="img"><a href="daviplata?accion=index">
            <img src='
              data:image/png;base64,' />
          </a>
        </div>
        <h3><a href="daviplata?accion=index">HOME</a> </h3>
        <h1>MENU</h1>
      </div>
      <div class="centro">
        <div class="grid-container"> 
          <!-- menu  de opciones del usuario --> 

          <!-- imagen con el refferencia de ruta del servlet  " recargar"-->
          <div class="grid-item"><a href="daviplata?accion=recargar"><span style="color: #FFF;font-size:larger;font-family:'Arial';">RECARGAR</span></a> <br> <a href="daviplata?accion=recargar"><img id="img2" src="assets/images/recargar.png"></a>
              </div> 
                <!-- imagen con el refferencia de ruta del servlet  " consultar saldo "-->
          <div class="grid-item"><a href="daviplata?accion=consultar_saldo"><span style="color: #FFF;font-size:larger;font-family:'Arial';"> CONSULTAR
              SALDO</span></a><br> <a href="daviplata?accion=consultar_saldo"> <img id="img1"
                src="assets/images/consultar_saldo1.png"></a></div>
                  <!-- imagen con el refferencia de ruta del servlet  " retirar deinero "-->
          <div class="grid-item">  <a href="daviplata?accion=retirar"><span style="color: #FFF;font-size:larger;font-family:'Arial';">RETIRAR</span></a><br> 
            <a href="daviplata?accion=retirar"><img id="img3" src="assets/images/retirar_dinero.png"></a></div>
        </div>
      </div>
    </div>

    </div>

  </body>

  </html>